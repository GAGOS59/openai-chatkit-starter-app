...
if (lastAskedSud) {
  ...
  if (sud !== null) {
    ...
    // 1) Premier SUD saisi : on laisse l'API guider la suite
    if (previous === null) {
      // ne rien dire ici : l'API va proposer Setup/ronde
    } else {
      ...
      if (delta >= 2 && sud > 0) {
        setMessages((prev) => [
          ...prev,
          { role: "user", content: value },
          { role: "assistant", content: "Ton SUD a diminué d’au moins deux points. Nous poursuivons ce même ressenti." },
        ]);
      ...
      if (delta === 1) {
        setMessages((prev) => [
          ...prev,
          { role: "user", content: value },
          { role: "assistant", content: "Ton SUD n’a baissé que d’un point. Qu’est-ce qui pourrait maintenir ce ressenti ?" },
        ]);
        setInput("");
        return;
      }
      ...
      if (delta <= 0) {
        setMessages((prev) => [
          ...prev,
          { role: "user", content: value },
          { role: "assistant", content: "Le SUD n’a pas diminué. Explorons ce qui pourrait être à la racine : qu’est-ce que ça évoque pour toi ?" },
        ]);
        setInput("");
        return;
      }
