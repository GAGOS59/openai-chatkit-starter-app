...
function PromoCard() {
  ...
  const LS_KEY = "efty_promo_closed_v1";

  // init visible selon localStorage
  useEffect(() => {
  ...
  useEffect(() => {
    const prev = document.body.style.paddingBottom || "";
    if (visible && isMobile) {
      document.body.style.paddingBottom = "88px"; // ajuster si tu veux bandeau plus petit/grand
    } else {
      document.body.style.paddingBottom = prev;
    }
    return () => {
  ...
  const closePromo = () => {
    setVisible(false);
    try {
      localStorage.setItem(LS_KEY, "1"); // mémorise la fermeture
    } catch (e) {
      // ignore
    }
  };
  ...
  return (
    <aside
      className={
        // Desktop : sticky column ; Mobile : fixed bottom full-width
        "rounded-xl border bg-[#F3EEE6] text-[#0f3d69] p-4 shadow-sm " +
        "md:sticky md:top-6 " +
        "fixed left-0 right-0 bottom-0 md:relative md:w-auto z-50"
      }
