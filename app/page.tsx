  ...
  const send = async () => {
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        const reply = (data.reply as string) || '(aucune réponse)';
        add('bot', reply);
  ...
  return (
    <main style={{
      ...
      <div style={{ maxWidth: 720, margin: '0 auto', background: '#fff',
        border: '1px solid #0F3D69', borderRadius: 16, boxShadow: '0 6px 24px rgba(0,0,0,.08)', overflow: 'hidden' }}>
        <header style={{ padding: 20, borderBottom: '1px solid #0F3D69' }}>
          <h1 style={{ margin: 0, fontSize: 22 }}>Guide EFT – École EFT France</h1>
          <p style={{ margin: '6px 0 0 0', fontSize: 14, opacity: .85 }}>
        ...
        <div style={{ display: 'flex', gap: 8, borderTop: '1px solid #0F3D69', padding: 12, background: '#fff' }}>
          ...
          />
          <button onClick={send}
            style={{ padding: '12px 16px', background: '#0F3D69', color: '#fff', border: 'none', borderRadius: 10, fontSize: 16, cursor: 'pointer' }}>
            Envoyer
          </button>
        </div>
      </div>
